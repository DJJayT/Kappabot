...
class SlashCommands extends Command {
    protected function configure(): void {
        $this
            ...
            ->addArgument('slash-commands', InputArgument::IS_ARRAY | InputArgument::OPTIONAL,
                'The slash-command(s) to perform said action on');
    }
        ...
        $env->discord->on("ready", static function () use ($action, $output, $commands, $verb) {
            ...
            if ($action === 'deleteall') {
                ...
                foreach ($commands as $i => $value) {
                    ...
                    $command::$action()
                        ->done(static function ($commandName) use ($output, $close, $discord) {
